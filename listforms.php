<?php // $Id: listforms.php,v 1.16 2012/11/14 10:58:53 shtifanov Exp $
    ...
    if ($action == 'excel') 	{
        $sql = "SELECT r.id, r.shortname FROM mdl_monit_form f
            inner join mdl_monit_razdel r on f.id=r.formid
            where f.levelmonit = 'rating_dou'";
        $rkps = get_records_sql_menu($sql);  
        $datefrom = get_date_from_month_year($nm, $yid);
        print_excel_header('rating_'.$oid.'_'.$nm.'_all');
		create_excel_workbook();
	    foreach($rkps as $rkp)	{
			// print_excel_form('rkp_prr_ro', $datefrom);
			print_excel_form($rkp, $datefrom, 'rating', $rid, $oid, $yid, '', true);
		}
...
*/
	...
	if ($rid != 0 && $oid != 0)   {
    	...
    	if (!$edit_capability_region)	{   //  && !$edit_capability_rayon  ОТКРЫТИЕ ДОСТУПА ДЛЯ МУНИЦИПАЛЬНЫХ ОПЕРАТОРОВ
    		if ($timedenied > $REGIONCRITERIA->doutimeaccessdenied)	{  
    	...
    	if ($view_capability || $edit_capability_region || $edit_capability_rayon)	{
       	    ...
       	    $table = table_begindata($rid, $oid, $yid, $nm, $typeou, $outype);
           	// print_table($table);
           	print_color_table($table);
    
        	$options = array('action'=> 'excel', 'rid' => $rid, 'oid' => $oid, 'yid' => $yid, 
        					 'fid' => $fid,  'nm' => $nm, 'typeou' => $typeou,  'sesskey' => $USER->sesskey);
           	echo '<center>';
        ...
        if ($curryearid == $yid)    {
            $strtimeclose = date('d.m.Y\г\. \в H:i', $REGIONCRITERIA->doutimeaccessdenied);
    	   notify("<b><i>Внимание! $strtimeclose доступ к исходным данным системы рейтингования ДОУ будет закрыт!</i></b>");
        }   
    }
    print_footer();


function table_begindata($rid, $oid, $yid, $nm, $typeou, $outype)	
{
	global $CFG, $view_capability, $edit_capability_region, $edit_capability_rayon;

    // $rkps = array('rating_dou_');
    $sql = "SELECT r.id, r.shortname FROM mdl_monit_form f
            inner join mdl_monit_razdel r on f.id=r.formid
            where f.levelmonit = 'rating_dou'";
    $rkps = get_records_sql_menu($sql);        
    ...
    foreach($rkps as $rkp)	{
	    ...
	    $strsql = "SELECT * FROM {$CFG->prefix}monit_rating_listforms
 		   		   WHERE ({$outype->idfieldname}=$oid) and (shortname='$rkp') and (datemodified=$datefrom)";
